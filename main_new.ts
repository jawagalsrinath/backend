...
class Parser {
  ...
  getFields(fields: FieldType[], retJson: any = {}, path?: string) {
    ...
    for (let field of fields) {
      try {
        ...
        // console.log(`Error: Cannot Create Object ${err}`);
      }
    }
    return retJson;
  }
  ...
  addValidation() {
    ...
    this.parsedObject["Validation"] = this.getFields(
      [{
        name: "Type",
        fieldName: "type",
      }],
    ...
    this.parsedObject["Validation"] = {
      ...this.getFields(validationFields),
      ...this.parsedObject["Validation"],
    };
    return this;
  }
...
const constructObject = (obj: any, key: string): any => {
  const p = new Parser();
  return p
    .setJson(obj)
    .extractFields([{
...
const getProperties = (json: any) => {
  ...
  for (let x of Object.keys(objectProperties)) {
    console.log(constructObject(objectProperties[x], x));
  }
};
